...
local teleport = {x=1349, y=1504, z=7}
   ...
   if isPlayerGuild(cid) == TRUE then
      if sto_ativ == 1 or sto_ativ == -1 then
         ...
         guild_sto = getGlobalStorageValue(storages[3])
            ...
            if guild ~= guild_sto then            
               ...
               doBroadcastMessage("[King Baiak Castle] O(a) "..getCreatureName(cid).." acabou de conquistar o castelo "..castle_one_name.." para a guild \""..guildname.."\" pela "..sto_vezes_total.."ª vez(s). Premio: 10% a mais de exp.", 22)
      ...
      doPlayerSendCancel(cid,"[King Baiak Castle] O sistema está desativado")
      end      
   else
   doPlayerSendCancel(cid,"[King Baiak Castle] É nescessario ter guild para dominar o castelo!")
   doSendMagicEffect(position, 2)
   end
end
