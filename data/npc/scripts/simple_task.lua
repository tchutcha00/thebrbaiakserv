		...
		if task_sys[task] then
			if getPlayerStorageValue(cid, task_sys[task].start) <= 0 then
				if getPlayerLevel(cid) >= task_sys[task].level then
					...
					npcHandler:say("[Task System] Parabens, agora voce este participando da Task do "..task_sys[task].name.." e devera matar "..task_sys[task].count.." desta lista: "..getMonsterFromList(task_sys[task].monsters_list)..". "..(#task_sys[task].items > 0 and "Ah e por favor me traga "..getItemsFromList(task_sys[task].items).." para mim." or "").."" , cid)
				else
				...
				npcHandler:say("Desculpe, Mas voce atualmente este na task "..task_sys[task].name..". Voce pode {entregar} caso ja tenha terminado.", cid)
			end
		else
