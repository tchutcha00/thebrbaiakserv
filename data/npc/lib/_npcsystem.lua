...
if(NpcSystem == nil) then
	...
	function NpcSystem.parseParameters(npcHandler)
		-- coinId
		local ret = NpcSystem.getParameter('coinId')
		if ret then
			npcHandler.coinId = tonumber(ret)
		end
		...
		-- coinId
		local ret = NpcSystem.getParameter('message_buy_coinid')
		if ret then
			npcHandler:setMessage(MESSAGE_BUY_COINID, ret)
		end
		local ret = NpcSystem.getParameter('message_onbuy_coinid')
		if ret then
			npcHandler:setMessage(MESSAGE_ONBUY_COINID, ret)
		end
		local ret = NpcSystem.getParameter('message_sell_coinid')
		if ret then
			npcHandler:setMessage(MESSAGE_SELL_COINID, ret)
		end
		local ret = NpcSystem.getParameter('message_onsell_coinid')
		if ret then
			npcHandler:setMessage(MESSAGE_ONSELL_COINID, ret)
		end
