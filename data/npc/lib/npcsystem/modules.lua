...
if(Modules == nil) then
	...
	function ShopModule:addBuyableItem(names, itemid, cost, subType, realName, periodItem)
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		local coinId = self.npcHandler.coinId
		local parseInfo = {
			[TAG_PLAYERNAME] = getPlayerName(cid),
			[TAG_ITEMCOUNT] = amount,
			[TAG_TOTALCOST] = totalCost,
			[TAG_ITEMNAME] = shopItem.name,
			[TAG_COINIDNAME] = coinId and getItemInfo(coinId).name or 'Token'
		}

		local needMessage
		if coinId then
			if getPlayerItemCount(cid, coinId) < totalCost then
				needMessage = MESSAGE_NEEDCOINID
			end
		else
			if (getPlayerMoney(cid) + getPlayerBalance(cid)) < totalCost then
				needMessage = MESSAGE_NEEDMONEY
			end
		end
		if needMessage then
			local msg = self.npcHandler:getMessage(needMessage)
			doPlayerSendCancel(cid, self.npcHandler:parseMessage(msg, parseInfo))
			return false
		end

		local subType = shopItem.subType or 1
		local a, b = doNpcSellItem(cid, itemid, amount, subType, ignoreCap, inBackpacks, backpack, shopItem.period)
		if(a < amount) then
			...
			if a > 0 then
				if coinId then
					if not doPlayerRemoveItem(cid, coinId, ((a * shopItem.buy) + (b * 20))) then
						return false
					end
				else
			...
			local msg = self.npcHandler:getMessage(coinId and MESSAGE_ONBUY_COINID or MESSAGE_BOUGHT)
			if coinId then
				if not doPlayerRemoveItem(cid, coinId, totalCost) then
					return false
				end
			else
	...
	function ShopModule:callbackOnSell(cid, itemid, subType, amount, ignoreEquipped, dummy)
		...
		if(doPlayerRemoveItem(cid, itemid, amount, subType, ignoreEquipped)) then
			local msg
			if coinId then
				msg = self.npcHandler:getMessage(MESSAGE_ONSELL_COINID)
				doPlayerAddItem(cid, coinId, amount * shopItem.sell)
			else
