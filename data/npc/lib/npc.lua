...
function doNpcSellItem(cid, itemid, amount, subType, ignoreCap, inBackpacks, backpack, periodItem)
	...
	if inBackpacks then
		...
		for i = 1, amount do
			...
			if periodItem and periodItem > 0 then
				doItemSetPeriod(item, periodItem)
			end
