function onSay(cid, words, param, channel)
    ...
    local failout = {"7735", "8926", "8932", "8929", "5907"}
    for i = 1, #failout do
    	local itemName = getItemInfo(tonumber(failout[i])).name
        if string.find(param:lower(), failout[i]) or itemName:lower() == param:lower() then
            doPlayerSendCancel(cid,"Você não pode criar este item.")
            return true
        	end 
   		end
	end 
