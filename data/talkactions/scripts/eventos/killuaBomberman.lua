...
if(not checkExhausted(cid, 666, 1)) then
	...
	local bombStoragee = 722602
	if getCreatureStorage(cid, bomberman.inGameStorage) > -1 then
		local bombs = getCreatureStorage(cid, bomberman_bombStoragee)
		local bombLimit = getCreatureStorage(cid, bomberman_bombLimitStorage)
		if bombs <= ((bombLimit * 3) + 997) then
			local pos = getThingPos(cid)
			local radius = getCreatureStorage(cid, bomberman_radiusStorage)
			local bomb_item = doCreateItem(bomberman_bombId, 1, pos)
			doItemSetAttribute(bomb_item, "radius", radius)
			sendCountDown(pos,3)
			addEvent(decreaseStorage, 1100, cid, 3)
			addEvent(explode, 3000, pos, radius)
			doCreatureSetStorage(cid, bomberman_bombStorage, bombs + 3)
		else
