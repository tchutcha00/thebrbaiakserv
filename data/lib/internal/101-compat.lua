...
registerPlayerEvent = registerCreatureEvent
unregisterPlayerEvent = unregisterCreatureEvent
...
function Creature:getGuid() return getPlayerGUID(self.cid) end
...
function Creature:getPosition() return getThingPos(self.cid) end
...
function Creature:registerEvent(event) registerCreatureEvent(self.cid, event) end
function Creature:unregisterEvent(event) unregisterCreatureEvent(self.cid, event) end

-- Player set functions
function Player:setLossPercent(skillId, loss) doPlayerSetLossPercent(self.cid, skillId, loss) end
...
function Player:getGroup() return getPlayerGroup(self.cid) end
...
function Player:getStorageValue(storage) return getPlayerStorageValue(self.cid, storage) end
...
Item = {}
setmetatable(Item, {__call = function(self, uid) self.uid = uid return self end})

function Item:remove() return doRemoveItem(self.uid) end
function Item:transform(itemid) return doTransformItem(self.uid, itemid) end
function Item:setAttribute(type, value) return doItemSetAttribute(self,uid, type, value) end